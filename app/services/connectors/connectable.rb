module Connectors::Connectable
  ...
  def set_connector
    @connector = Connector.find_by(user_id: @params[:user_id], bank: @params[:bank]) || Connector.build(@params)
  ...
  def wait_for_connector_prompt(attr, max_tries: 5, current_try: 0, delay: 10)
    ...
    sleep(delay)
    wait_for_connector_prompt(attr, max_tries: max_tries, current_try: current_try + 1, delay: delay)
  end
