...
function reqTransactionListener(msg) {
...
function reqAccountsListener(msg) {
  const response = JSON.parse(this.responseText);
...
port.onMessage.addListener((msg) => {
  ...
  if (msg.name === 'pull_accounts') {
    ...
    req.addEventListener('load', reqAccountsListener(msg));
    req.open(
      'GET',
  ...
  if (msg.name === 'pull_transactions_deposit_account') {
    const req = new XMLHttpRequest();
    req.addEventListener('load', reqTransactionListener(msg));
    req.open(
      'GET',
