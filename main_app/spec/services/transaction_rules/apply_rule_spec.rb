...
RSpec.describe TransactionRules::ApplyRule do
  ...
  context 'when conditions have groups' do
    let!(:transaction_1) do
      create(:transaction, account: account, category: category_1, is_credit: false)
    end
    let!(:transaction_2) do
      create(:transaction, account: account, category: category_4, is_credit: true,
        description: 'jamal is my kamal')
    end
    let!(:transaction_3) do
      create(:transaction, account: account, category: category_4, is_credit: true,
        description: 'jamal is my hamal')
    end
