...
RSpec.describe Transactions::SetSuggestedCategories, type: :service do
  ...
  let(:transaction) do 
    ...
    transaction.to_neighbors << create(:transaction, account: account, category: user.categories.sample)
    transaction.to_neighbors << create(:transaction, account: account, category: user.categories.sample)
    transaction.to_neighbors << create(:transaction, account: account, category: user.categories.sample)
    transaction.save!
    transaction
  ...
  before do
    expect(RubyLLM).to receive_message_chain(:chat, :with_request_options, :ask, :content)
