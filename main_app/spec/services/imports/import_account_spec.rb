...
describe Imports::ImportAccount, type: :service do
  ...
  context "when account does not exist" do
    it "creates new account" do
      expect { described_class.new(connector, account_params).call }.to change { Account.count }.by(1)
    end
  end
  ...
  context "when account exists" do
    ...
    it "updates account" do
      expect { described_class.new(connector, account_params).call }.to change { Account.count }.by(0)
