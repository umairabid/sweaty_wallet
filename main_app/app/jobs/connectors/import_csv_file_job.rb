class Connectors::ImportCsvFileJob < ApplicationJob
  ...
  def perform(file_import)
    ...
    connector = Connector.find_or_create_by!(user: file_import.user, bank:) do |conn|
    ...
    accounts.each do |account_id|
      ...
      Imports::ImportTransactions.call(account, account_transactions[account_id].map do |t|
                                                  t.to_h.symbolize_keys
                                                end)
    end
